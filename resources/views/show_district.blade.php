...
<!-- Head -->
<head>
    <title>Lịch cúp điện {{$provinceDistricts->district_name}} - {{$provinceDistricts->province->name}} hôm nay ngày mai và tuần ngày - LichCupDien.org </title>
    <meta charset="UTF-8">
    <meta name="description" content="Lịch cúp điện {{$provinceDistricts->district_name}} - {{$provinceDistricts->province->name}} mới nhất hôm nay ngày mai và tuần này. Cập nhật lịch cắt điện nhanh nhất tại {{$provinceDistricts->district_name}} - {{$provinceDistricts->province->name}}">
    <meta name="keywords" content="lich cup dien, lich cup dien thanh pho long xuyen an giang, lich cup dien hom nay, lich cup dien moi nhat, lich cup dien ngay mai">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <link rel="icon" type="image/x-icon" href="/lichcupdien-favicon.png">
    <meta property="og:type" content= "website" />
    <meta property="og:site_name" content="Lịch cúp điện {{$provinceDistricts->district_name}} - {{$provinceDistricts->province->name}}" />
    <meta property="og:url" content="https://lichcupdien.org/{{$provinceDistricts->slug_district}}" />
    ...
    <link rel="canonical" href="https://lichcupdien.org/{{$provinceDistricts->slug_district}}" />
...
<div class="w3-content" style="max-width:1140px">
    <div class="lcd-breadcrumbs">
        <ul class="breadcrumbs">
            ...
            <li class="breadcrumbs_item">
                <a href="/{{$provinceDistricts->province->slug}}" class="breadcrumbs_link">{{$provinceDistricts->province->name}}</a>
            </li>
            <li class="breadcrumbs_item">
                <span class="breadcrumbs_link breadcrumbs_link_active">{{$provinceDistricts->district_name}} </span>
            </li>
        </ul>
    </div>
    <div class="w3-row">
        <div class="w3-col l8 s12">
            <div class="lcd-card-4 lcd_main_margin w3-white">
                <div class="w3-container">
                    <h1 style="font-weight:bold;">Lịch cúp điện {{$provinceDistricts->district_name}} - {{$provinceDistricts->province->name}} hôm nay ngày mai và tuần ngày</h1>
                    <h5>Cập nhật ngày {{ date('d/m/Y') }}</h5>
                    <p>Cập nhật lịch tạm ngừng cung cấp điện Điện lực {{$provinceDistricts->district_name}} - {{$provinceDistricts->province->name}} mới nhất.</p>
                    @php
                        $months = [
                    ...
                    @endphp
                    <p>Thông tin lịch cúp điện {{$provinceDistricts->district_name}} - {{$provinceDistricts->province->name}} {{ $months[$currentMonth] }} và lịch cúp điện {{ $months[$nextMonth] }} chính xác để mọi người, cơ quan, doanh nghiệp sắp xếp công việc, chủ động trong hoạt động tiêu dùng sản xuất kinh doanh.</p>
                </div>
                <div class="w3-container">
                    <img src="/lib/img/{{$provinceDistricts->province->slug}}.jpg" alt="Lịch cúp điện mới nhất hôm nay" style="width:100%; margin-bottom: 5px;">
                    <h2 style="font-weight:bold">Thông báo lịch cúp điện {{$provinceDistricts->district_name}} - {{$provinceDistricts->province->name}} có tác dụng gì?</h2>
                    <p>Việc xảy ra tình trạng mất điện tại {{$provinceDistricts->district_name}} - {{$provinceDistricts->province->name}} có thể có rất nhiều nguyên nhân. Các yếu tố có thể gây mất điện như: thiên tai, lũ lụt, bão, hoặc do đơn vị quản lý thực hiện thi công sửa chữa bảo trì đường dây lưới điện</p>
                    <p>Dù cúp điện do bất kỳ nguyên nhân gì thì cũng sẽ gây ảnh hưởng rất lớn đến cuộc sống sinh hoạt và sản xuất của người dân thuộc khu vực mà Điện lực {{$provinceDistricts->district_name}} - {{$provinceDistricts->province->name}} quản lý.</p>
                    <p>Mất điện hay cúp điện được hiểu là điện áp đột ngột hạ xuống còn 0V. Khi đó, sinh hoạt bình thường của người dân bị ảnh hưởng. Khi nắng nóng, không thể sử dụng các thiết bị điện như quạt điện, hay điều hòa,... Nông dân không thể sử dụng máy bơm nước điện để tưới cây. Nhà máy, xí nghiệp không thể sản xuất như bình thường. Nên mọi người cần nắm rõ lịch cắt điện tại {{$provinceDistricts->district_name}} - {{$provinceDistricts->province->name}} để chủ động được sản xuất và sinh hoạt hàng ngày.</p>
                    <p>Để mọi người thuận tiện tra cứu lịch cúp điện {{$provinceDistricts->district_name}} - {{$provinceDistricts->province->name}} và các khu vực, tỉnh thành xung quanh, trang web lichcupdien.org ra đời với mong muốn giúp mọi người có nơi tra cứu một cách nhanh chóng và chính xác lịch cắt điện {{$provinceDistricts->district_name}} - {{$provinceDistricts->province->name}}. Khi nắm rõ được thông tin lịch mất điện {{$provinceDistricts->district_name}} - {{$provinceDistricts->province->name}} sẽ giúp quý khán giả có tâm lý thoải mái, không bị bất ngờ khi sự cố cắt điện diễn ra.</p>
                    <div>
                        <div>
                            <span style="font-size: 17px; font-weight: 500;">Xem thêm:</span>
                        </div>
                        <div>
                            <a href="/{{$provinceDistricts->province->slug}}" style="text-decoration: underline;">Lịch cúp điện {{$provinceDistricts->province->name}}</a>
                        </div>
                    </div>
                    <h2 style="font-weight:bold">Lịch cúp điện {{$provinceDistricts->district_name}} - {{$provinceDistricts->province->name}} những ngày tới</h2>
                    <h3 style="font-size:16px" class="tab-items-title-bold tab-items-red">Lịch cúp điện {{$provinceDistricts->district_name}} - {{$provinceDistricts->province->name}} ngày </h3>
                    <hr/>
                    <div class="lcd_detail_wrapper">
                        <div class="new_lcd_wrapper">
                            ...
                            <div class="item_content_lcd_wrapper"><span class="content_item_content_lcd_wrapper item_txt_bold">Điện lực Long Xuyên</span></div>
                        </div>

                        <div class="new_lcd_wrapper">
                            <div class="item_lcd_wrapper"><span class="title_item_lcd_wrapper">Ngày:</span></div>
                            <div class="item_content_lcd_wrapper"><span class="content_item_content_lcd_wrapper item_txt_bold item_txt_red">13 tháng 3 năm 2024</span></div>
                        </div>

                        <div class="new_lcd_wrapper">
                            <div class="item_lcd_wrapper"><span class="title_item_lcd_wrapper">Thời gian:</span></div>
                            <div class="item_content_lcd_wrapper"><span class="content_item_content_lcd_wrapper">Từ <span class="item_lcd_time">08:00</span> đến <span class="item_lcd_time">13:30</span></span></div>
                        </div>

                        <div class="new_lcd_wrapper">
                            <div class="item_lcd_wrapper"><span class="title_item_lcd_wrapper">Khu vực:</span></div>
                            <div class="item_content_lcd_wrapper"><span class="content_item_content_lcd_wrapper">Một phần Phường Bình Đức (khu vực ngọn Trà Ôn từ trạm Y Tế đến cầu Sáu Hành) - TP. Long Xuyên</span></div>
                        </div>

                        <div class="new_lcd_wrapper">
                            <div class="item_lcd_wrapper"><span class="title_item_lcd_wrapper">Lý do:</span></div>
                            <div class="item_content_lcd_wrapper"><span class="content_item_content_lcd_wrapper">Thí nghiệm, sửa chữa bảo dưỡng Trung, hạ áp</span></div>
                        </div>
                        <div class="new_lcd_wrapper">
                            <div class="item_lcd_wrapper"><span class="title_item_lcd_wrapper">Trạng thái:</span></div>
                            <div class="item_content_lcd_wrapper"><i class="fa fa-check-circle" style="color: #00c590;"></i><span class="content_item_content_lcd_wrapper lcd_check_trang_thai">Đã duyệt</span></div>
                        </div>
                    </div>
                    <hr />
                    ...
                    <h2 style="font-weight:bold">Những cách tra cứu lịch cúp điện {{$provinceDistricts->district_name}} - {{$provinceDistricts->province->name}} mới nhất</h2>
                    ...
                    <p>Cách tra cứu lịch cúp điện {{$provinceDistricts->district_name}} - {{$provinceDistricts->province->name}} bằng ứng dụng cũng vô cùng đơn giản.</p>
                    <p>Mọi người chỉ cần tải ứng dụng về máy điện thoại, đăng ký tài khoản, sau đó chọn mục "Lịch ngừng giảm cung cấp điện" là có thể xem được lịch mất điện {{$provinceDistricts->district_name}} - {{$provinceDistricts->province->name}}</p>
                    <h3>3. Tra cứu lịch mất điện trên ứng dụng Zalo</h3>
                    <p>Để tìm lịch ngừng giảm cung cấp điện {{$provinceDistricts->district_name}} - {{$provinceDistricts->province->name}} trên Zalo. Bạn cần đăng nhập tài khoản Zalo. Nhấn tìm Điện lực miền Nam, hoặc Điện lực miền Trung, hoặc Điện lực miền Bắc, nhấn Quan tâm.</p>
                    <p>Ở khung chat, các bạn nhấn chữ tra cứu, sau đó chọn Lịch cúp điện là có thể xem lịch cắt điện tại khu vực mình đang ở.</p>
                    <h3>4. Thông báo lịch cúp điện {{$provinceDistricts->district_name}} - {{$provinceDistricts->province->name}} trên website LichCupDien.org</h3>
                    <p>Với cách này, các thao tác thực hiện là đơn giản nhất.</p>
                    <p>Mọi người chỉ cần truy cập trang web LichCupDien.org rồi chọn khu vực bạn sinh sống. Sau đó là bạn có thể xem ngay lịch cúp điện {{$provinceDistricts->district_name}} - {{$provinceDistricts->province->name}} một cách vô cùng đơn giản.</p>
                    ...
                    <h2 style="font-weight:bold">Tổng đài chăm sóc khách hàng Điện lực {{$provinceDistricts->district_name}} - {{$provinceDistricts->province->name}}</h2>
                    <p>Khi có sự cố về điện tại địa phương, hoặc khách hàng có nhu cầu tra cứu, yêu cầu đăng ký điện,... Quý khách hàng có thể liên hệ đến tổng đài Điện lực {{$provinceDistricts->district_name}} - {{$provinceDistricts->province->name}} gần nhất theo các cách sau:</p>
                    <h3>Tổng đài Điện lực Miền Bắc:</h3>
                    <p>Quý khách hàng có thể gọi ngay đến số điện thoại điện lực miền Bắc: 1900 6769 để được hỗ trợ các vấn đề về lịch cúp điện {{$provinceDistricts->district_name}} - {{$provinceDistricts->province->name}} mà bạn đang gặp phải.</p>
                    ...
                    <p>Bạn có nhu cầu tra cứu lịch cúp điện {{$provinceDistricts->district_name}} - {{$provinceDistricts->province->name}}, bạn có thể gọi ngay tổng đài hỗ trợ điện lực tại số 1900 1006 hoặc 1900 9000. Với 2 số điện thoại này, bạn sẽ được nhân viên chăm sóc khách hàng của EVN hỗ trợ nhiệt tình, giúp bạn khác phục các vấn đề một cách nhanh chóng.</p>
                    ...
                    <p>Với những thông tin trên đây. Mong rằng quý khách hàng sẽ có được những sự hỗ trợ tốt nhất từ chúng tôi. Khi có sự cố hoặc cần hỗ trợ thông báo lịch cúp điện {{$provinceDistricts->district_name}} - {{$provinceDistricts->province->name}}, đừng ngần ngại gọi ngay cho chúng tôi. Chúng tôi sẽ giúp bạn giải quyết vấn đề nhanh chóng, an toàn và hiệu quả.</p>
                </div>
            </div>
            <!-- END BLOG ENTRIES -->
        </div>
        <div class="w3-col l4">
            <div class="lcd-card lcd_main_margin">
                <div class="w3-container m-sidebar">
                    <div class="lcd-sidebar-title">
                        <h4 class="lcd-sidebar-title-bold">Lịch cúp điện các quận huyện tại {{$provinceDistricts->province->name}}</h4>
                    </div>
                </div>
                    ...
                    <div class="">
                        @foreach($districts as $district)
                            <h5 class="lcd-sidebar-relate">
                                <a href="/tinh/{{$district->slug_district}}" class="related-area-main">
                                    Lịch cúp điện {{$district->district_name}}
                                </a>
                            </h5>
                        @endforeach
                    </div>
            ...
            <div class="lcd-card lcd_main_margin">
                ...
                <div class="w3-container w3-white side-text">
                    @if($provinceDistricts->province->region_id == 1)
                        ...
                        @endforeach
                    @elseif($provinceDistricts->province->region_id == 2)
